...
function home() {
 ...
 <section class="hero clay"><div class="hero-copy"><span class="pill">${icon('sparkle')}작은 시작, 깔끔한 정리</span><h2>흩어진 것들을 모아,<br>한눈에 쏙.</h2><p>수합부터 시뮬레이션까지.<br>나만의 작업 공간을 차근차근 채워보세요.</p>${link('collection',`수합 공간 살펴보기 ${icon('arrow')}`,'button primary')}</div><div class="hero-stack" aria-label="수합과 시뮬레이션 기능 준비 중"><div class="stack-card stack-back"><span class="mini-label">SIMULATION</span><div class="stack-title">가능성을 그려요 ${icon('sliders')}</div><div class="bars"><i></i><i></i><i></i><i></i><i></i></div></div><div class="stack-card stack-front"><span class="mini-label">COLLECTION</span><div class="stack-title">한곳에 모아요 <span class="tiny-icon">${icon('folder')}</span></div><div class="sample-line"><span class="tick">${icon('check')}</span><span>응답과 자료</span></div><div class="sample-line"><span class="tick">${icon('check')}</span><span>아이디어와 의견</span></div><span class="sample-note">함께 채워갈 공간</span></div><span class="float-chip">${icon('layers')}차곡차곡, 모아</span></div></section>
 <section class="stats" aria-label="워크스페이스 현황"><div class="stat"><span class="stat-icon mint">${icon('folder')}</span><div><span>전체 수합</span><strong>0 <small>개</small></strong></div><span class="stat-note">첫 수합을 기다려요</span></div><div class="stat"><span class="stat-icon blue">${icon('sliders')}</span><div><span>시뮬레이션</span><strong>0 <small>개</small></strong></div><span class="stat-note">곧 만나요</span></div><div class="stat"><span class="stat-icon peach">${icon('clock')}</span><div><span>진행 중인 작업</span><strong>0 <small>개</small></strong></div><span class="stat-note">아직 작업이 없어요</span></div></section>
 <section><div class="section-heading"><h2>무엇부터 시작할까요?</h2><span>나에게 필요한 공간을 골라보세요</span></div><div class="module-grid">${modules.map(moduleCard).join('')}<a class="guide-card" href="#/guide"><span class="guide-icon">${icon('book')}</span><h3>모아가 처음이라면</h3><p>공간을 둘러보고<br>앞으로의 사용 흐름을 알아보세요.</p><span class="card-link">이용 가이드 ${icon('arrow')}</span></a></div></section>
...
function guide() {return `<section class="welcome"><div><span class="eyebrow">QUICK GUIDE</span><h1>모아, 이렇게 둘러보세요.</h1><p>필요한 것을 모으고 살펴보는 작은 워크스페이스.</p></div></section><div class="guide-list">${[['01','홈에서 전체 흐름 보기','워크스페이스 현황과 각 기능으로 이동하는 바로가기를 확인하세요.','home'],['02','수합 공간 둘러보기','앞으로 응답과 자료를 모을 공간이에요. 현재는 준비 화면을 제공해요.','collection'],['03','시뮬레이션 공간 둘러보기','앞으로 조건에 따른 결과를 살펴볼 공간이에요. 현재는 준비 화면을 제공해요.','simulation']].map(([n,t,d,id])=>`<a class="guide-step clay" href="#/${id}"><span class="step-number">${n}</span><div><h2>${t}</h2><p>${d}</p></div>${icon('arrow')}</a>`).join('')}</div><div class="info-note">${icon('layers')}<p>기본 구조 버전 · 로그인, 서버 연결, 데이터 저장 기능은 아직 연결되지 않았어요.</p></div>`;}
function render() {
 const requested=location.hash.replace('#/','')||'home';
 const route=navigation.some(([id])=>id===requested)?requested:'home';
 document.title=`${navigation.find(([id])=>id===route)[1]} | 모아`;
 app.innerHTML=`<a class="skip-link" href="#main">본문으로 이동</a><aside class="sidebar"><a class="brand" href="#/home"><span class="brand-symbol">m</span><span>모아<span class="brand-sub">moa workspace</span></span></a><div class="workspace-switch"> <span class="workspace-avatar">M</span><div>나의 워크스페이스<small>Personal space</small></div></div><div class="nav-caption">WORKSPACE</div><nav aria-label="주 메뉴">${navigation.map(([id,label,i])=>link(id,`${icon(i)}<span>${label}</span>${id==='simulation'?'<span class="soon">SOON</span>':''}`,`nav-link ${id===route?'active':''}`).replace('class=',`${id===route?'aria-current="page" ':''}class=`)).join('')}</nav><div class="sidebar-bottom"><span class="bottom-mark">${icon('sparkle')}</span><strong>조금씩, 더 편리하게</strong><p>새로운 기능을 담아갈<br>우리의 작은 작업 공간.</p><span class="version">모아 · v0.1</span></div></aside><div class="page"><header class="topbar"><div class="breadcrumb">워크스페이스 <span>/</span> <strong>${navigation.find(([id])=>id===route)[1]}</strong></div><span class="top-label">차곡차곡 쌓이는 가능성</span><span class="profile" aria-label="개인 워크스페이스">M</span></header><main id="main" tabindex="-1">${route==='home'?home():route==='guide'?guide():feature(route)}<footer><span>© ${new Date().getFullYear()} 모아</span><span>모으고, 살펴보고, 한 걸음 더.</span></footer></main></div>`;
 window.scrollTo(0,0);
}
